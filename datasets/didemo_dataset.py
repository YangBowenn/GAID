...
class DiDeMoDataset(Dataset):
    ...
    def __init__(self, config: Config, split_type='train', img_transforms=None):
        ...
        if split_type == 'train':
            ...
            self.video_dir = pth + 'train_videos_mp4/'
            self.audio_dir = pth + 'train_videos_audios_features/'
        elif split_type == 'test':
            self.label_csv = pth + 'didemo_test_label.csv'
            self.video_dir = pth + 'test_videos_mp4/'
            self.audio_dir = pth + 'test_videos_audios_features/'
        else:
        ...
        self.sample_type = config.video_sample_type
    ...
    def _get_rawvideo(self, video_path, s, e):
        ...
        imgs, _ = VideoCapture.load_frames_from_video(video_path, self.config.num_frames, self.config.video_sample_type)
        return imgs
    ...
    def __getitem__(self, index):
        ...
        video_path = self.video_dir + video_path.split('/')[-1].split('.')[0] + '.mp4'
        audio_path = self.audio_dir + video_path.split('/')[-1].split('.')[0] + '.pt'
        ...
        if self.load_frames_from_preprocess_pth:
            ...
            imgs, _ = VideoCapture.load_frames_from_video(video_path, self.num_frames, self.sample_type)
