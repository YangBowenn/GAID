...
class BaseTrainer:
    ...
    def __init__(self, model, loss, metrics, optimizer, config: Config, writer=None):
        ...
        self.model = DDP(model, device_ids=[config.local_rank], output_device = config.local_rank, find_unused_parameters=True)
        self.eval_device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
